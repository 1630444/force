...
<div id="submission-details" class="submission-details">
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<td><?php echo (isset($this->_item->submission_created_by) && (int) $this->_item->submission_created_by > 0) ? JSNUniformHelper::getUserNameById($this->_item->submission_created_by) : "Guest"; ?></td>
		</tr>
		<tr>
			<th>
        ...
        if ($this->_formPages)
        {
            foreach ($this->_formPages as $formPages)
            {
                ...
                foreach ($pageContent as $fields)
                {
                    ...
                    {
                        if ($fields->type != 'static-content' && $fields->type != 'google-maps')
                        {
                            $label = $fields->label != '' ? $fields->label : $fields->identify;
                            $submissionEdit .= '<div class="control-group ">
												<label class="control-label">' . rtrim($label, ':') . ':</label>
												<div class="controls">';
                            $contentField = "";
                            $contentFieldEdit = "";
                            $contentFieldDetail = "";
	                        ...
	                        // Show submission details only if the current field has submitted data.
	                        if (isset($submission->{$key}))
	                        {
		                        $submissionDetail .= '<dt>' . rtrim($label, ':') . ':</dt><dd id="' . $key . '">';
	                        }
	                        else
	                        {
		                        $submissionDetail .= '<dt class="hide">' . rtrim($label, ':') . ':</dt><dd id="' . $key . '" class="hide">';
	                        }
                            ...
                            {
                                $contentFieldEdit = htmlentities(str_replace('<span>N/A</span>', '', $contentFieldEdit), ENT_QUOTES, "UTF-8");
                                $submissionEdit .= "<input type=\"text\" name=\"submission[{$key}]\" dataValue=\"{$fields->id}\" typeValue=\"{$fields->type}\" class=\"jsn-input-xxlarge-fluid\" value=\"{$contentFieldEdit}\"" . (isset($submission->{$key}) ? '' : ' disabled') . ' />';
                            }
                        ...
                        } elseif (isset($submission->{$key}))
                        {
                            ...
                            elseif ($fields->type == 'google-maps')
                            {
