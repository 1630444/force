...
define([
    ...
    function ($, JSNVisualControls, JSNUniformDialogEdition, JSNModal) {
        ...
        JSNVisualDesign.initialize = function (language) {
            JSNVisualDesign.wrapper = $('<div class="jsn-element ui-state-default jsn-iconbar-trigger">'
                + '<div class="jsn-element-content"></div>'
                + '<div class="jsn-element-overlay"></div>'
                + '<div class="jsn-iconbar">'
                + '<a href="#" onclick="return false;" title="' + lang['JSN_UNIFORM_MOVE_ELEMENT'] + '" class="element-move' + ($("#jform_form_type").val() == 1 ? ' hidden' : '') + '">'
                + '<i class="icon-out-2"></i></a>'
                + '<a href="#" onclick="return false;" title="' + lang['JSN_UNIFORM_EDIT_ELEMENT'] + '" class="element-edit">'
                + '<i class="icon-pencil"></i></a>'
                + '<a href="#" onclick="return false;" title="' + lang['JSN_UNIFORM_DUPLICATE_ELEMENT'] + '" class="element-duplicate">'
                + '<i class="icon-copy"></i></a>'
                + '<a href="#" onclick="return false;" title="' + lang['JSN_UNIFORM_DELETE_ELEMENT'] + '" class="element-delete">'
            ...
            // Listen click event on document to handle click action on the icon to move element.
            if (!document._jsn_uniform_initialized_move_icon) {
                $(document).on('click', '.jsn-iconbar-trigger > .jsn-iconbar > .element-move', function(event) {
                    // Prepare page list.
                    var formHead = document.getElementById('form-design-header');
                    var pageList = document.querySelector('.jsn-page-list').cloneNode(true);

                    for (var i = 0; i < pageList.children.length; i++) {
                        // Set page name if missing.
                        if (!pageList.children[i].querySelector('a')) {
                            var a = document.createElement('a');
                            var f = pageList.children[i].querySelector('input');

                            a.href = '#';
                            a.textContent = f.value;

                            pageList.children[i].insertBefore(a, f);
                        }

                        // Do not show current page.
                        var page = pageList.children[i].id.replace(/^new_/, '');

                        if (page == formHead.getAttribute('data-value')) {
                            pageList.children[i].classList.add('hidden');
                        } else {
                            pageList.children[i].classList.remove('hidden');
                        }
                    }

                    // Convert page list to dropdown menu.
                    pageList.classList.add('dropdown-menu');
                    pageList.classList.remove('hide');

                    // Append dropdown menu to the move icon.
                    ...
                    // Show page list for selecting.
                    pageList.style.display = 'block';
                    pageList.style.position = 'absolute';

                    // Listen to click event on page list items.
                    $('.page-items', pageList).on('click', function(event) {
                        // Find element wrapper and target page.
                        var new_page;
                        var wrapper = event.target;

                        while (wrapper.nodeName != 'BODY') {
                            if (wrapper.classList && wrapper.classList.contains('jsn-element')) {
                                break;
                            }
                            else if (wrapper.classList && wrapper.classList.contains('page-items')) {
                                new_page = wrapper.querySelector('input').getAttribute('data-id');
                            }

                            wrapper = wrapper.parentNode;
                        }

                        if (!new_page || wrapper.nodeName == 'BODY') {
                            return;
                        }

                        // Get element data.
                        var data = $(wrapper).data('visualdesignElementData');
                        ...
                        $('#form-design-header .jsn-page-list .page-items[data-value="' + new_page + '"]')
                            .trigger('click');

                        // Automatically append the element to target page.
                        setTimeout(function moveElement() {
                            // Check page is finished loading.
                            var loading = document.querySelector('#form-design-content #page-loading');

                            if (loading.style.display == 'none') {
                                var element = JSNVisualDesign.createElement(data.type, data.options, data.id)[0];
                                var addButton = document.querySelectorAll('#form-container .jsn-column-content');
                                addButton = addButton[addButton.length - 1].querySelector('.jsn-add-more');

                                if (element && addButton) {
                                    addButton.parentNode.querySelector('.jsn-element-container').appendChild(element);
                                    JSNVisualDesign.savePage('add');
                                }
                            } else {
                ...
                $(document).on('mouseover', function(event) {
                    document.checkJsnUfPageSelectorTimer && clearTimeout(document.checkJsnUfPageSelectorTimer);

                    document.checkJsnUfPageSelectorTimer = setTimeout(function() {
                        document.hideJsnUfPageSelectorTimer && clearTimeout(document.hideJsnUfPageSelectorTimer);

                        // Check if the move icon is hovered?
                        var target = event.target;

                        while (target && target.nodeName != 'BODY') {
                            if (target.classList && target.classList.contains('element-move')) {
                                break;
                            }
                        ...
                        if (!target || target.nodeName == 'BODY') {
                            var pageList = document.querySelector('.element-move > .jsn-page-list');

                            if (pageList) {
                                document.hideJsnUfPageSelectorTimer = setTimeout(function () {
                                    // Remove page list.
                ...
                document._jsn_uniform_initialized_move_icon = true;
            }
        };
        ...
        JSNVisualDesign.eventChangeSlider = function(event) {
            if (event) {
                var step = document.getElementById('option-step-number');

                if (step) {
                    step = step.value.indexOf('.') > -1 ? parseFloat(step.value) : parseInt(step.value);

                    if (!isNaN(step) && step > 0) {
                        var value = event.target.value.indexOf('.') > -1
                            ? parseFloat(event.target.value)
                            : parseInt(event.target.value);

                        if (value % step != 0) {
                            alert('' + lang['JSN_UNIFORM_SLIDER_WRONG_DEFAULT_VALUE'] + '');

                            // Automatically correct the default value.
                            event.target.value = Math.round(value / step) * step;
                        }
                    }
                }
            }
        ...
        JSNVisualDesign.openOptionsBox = function (sender, type, params, action) {
            ...
            if (type == 'slider') {
                var defaultValue = document.getElementById('option-value-number');

                if (defaultValue) {
                    defaultValue.onchange = JSNVisualDesign.eventChangeSlider;
                }
            }
            if (type == "address") {
                JSNVisualDesign.eventChangeAddress();
        ...
        JSNVisualDesign.createControl = function (options, value, identify) {
            var templates = {
                ...
                'text': '<div class="controls {{if options.append}}input-append{{/if}}"><input {{if options.editionLimited}}disabled{{/if}} type="text" value="${value}" name="${options.name}" id="${id}" class="text jsn-input-xxlarge-fluid" /> {{if options.append}}<button type="button" class="btn btn-icon" {{if options.append.onClick}}onclick="${options.append.onClick}"{{/if}}>${options.append.text}</button>{{/if}}</div>',
                ...
                //'number': '<div class="controls"><input {{if options.editionLimited}}disabled{{/if}} type="number" value="${value}" name="${options.name}" id="${id}" class="number input-mini" {{if options.step}}step="${options.step}"{{/if}} /></div>',
        ...
        JSNVisualDesign.prototype = {
            ...
            setElements: function (elements) {
                ...
                // Show icons to move field to another page if there are more than 1 page available.
                setTimeout(function() {
                    if ($('.jsn-page-list > .page-items').length > 1) {
                        $('.jsn-iconbar-trigger > .jsn-iconbar > .element-move').removeClass('hidden');
                    } else {
                        $('.jsn-iconbar-trigger > .jsn-iconbar > .element-move').addClass('hidden');
                    }
                }, 100);
